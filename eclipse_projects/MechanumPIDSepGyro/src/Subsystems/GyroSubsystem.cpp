...
GyroSubsystem::GyroSubsystem() : frc::PIDSubsystem("test",0.035,0,0.015)  {//PID values
	SetAbsoluteTolerance(0.01);
	GetPIDController()->SetContinuous(false);
	SetOutputRange(-0.6,0.6);//Min and Max speed for the wheels
...
bool GyroSubsystem::AtSetpoint(){
	return (GetPosition()-GetSetpoint() < 4 &&GetPosition()-GetSetpoint() > -4)&&(RobotMap::drivetrainLB->GetSensorCollection().GetQuadratureVelocity()<5&&RobotMap::drivetrainLB->GetSensorCollection().GetQuadratureVelocity()>-5);
}
